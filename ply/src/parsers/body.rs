    ...
    opaque(
        move |f: &mut dyn FnMut(&mut dyn Parser<Input = _, Output = _, PartialState = _>)| match data_type {
            ...
            DataType::Uint8 => f(&mut no_partial(
                any().map(PropertyData::Uint8).expected("a binary u8 scalar"),
            )),
