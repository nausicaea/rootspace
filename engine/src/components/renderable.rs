...
impl Renderable<HeadlessRenderData> {
    pub fn from_mesh(backend: &HeadlessBackend, mesh: &Path, vs: &Path, fs: &Path, gs: Option<&Path>, dt: &Path, nt: &Path) -> Result<Self, Error> {
    ...
    pub fn from_text(backend: &HeadlessBackend, text: &str, font: &Path, vs: &Path, fs: &Path, gs: Option<&Path>) -> Result<Self, Error> {
...
impl From<HeadlessRenderData> for Renderable<HeadlessRenderData> {
    fn from(value: HeadlessRenderData) -> Self {
        Renderable { data: value, source: None }
    }
}
