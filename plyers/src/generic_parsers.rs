...
pub fn parse_unsigned_from_ascii<T, R>(file: &mut R) -> Result<T, Error>
where
    T: FromStr<Err = ParseIntError>,
    R: Read,
{
    let mut integer = String::new();
    parse_individual(file, |b| match b {
        0x09..=0x0d | 0x20 => Poll::Ready(Ok(())),
        b @ 0x30..=0x39 => {
            integer.push(char::from(b));
            Poll::Pending
        }
        b => Poll::Ready(Err(Error::UnexpectedByte(b))),
    })?;
    let integer: T = integer.parse()?;
    Ok(integer)
}

pub fn parse_word_from_ascii<R>(file: &mut R) -> Result<String, Error>
where
    R: Read,
{
...
pub fn parse_string_from_ascii<R>(file: &mut R) -> Result<String, Error>
where
    R: Read,
{
...
pub fn parse_from_lut<T, F, R>(file: &mut R, lut: &[&[u8]], mut func: F) -> Result<T, Error>
where
    R: Read,
...
pub fn parse_whitespace<R>(file: &mut R) -> Result<(), Error>
where
    R: Read,
{
    let byte = read_byte(file)?;
...
pub fn parse_individual<T, F, R>(file: &mut R, mut func: F) -> Result<T, Error>
where
    R: Read,
    F: FnMut(u8) -> Poll<Result<T, Error>>,
{
    loop {
        let byte = read_byte(file)?;
...
mod tests {
    ...
    proptest! {
        ...
        fn parse_string_from_ascii_terminates_at_the_first_newline(source in r"[[:ascii:]]+\n") {
            ...
            let r = parse_string_from_ascii(&mut stream);
        ...
        fn parse_word_from_ascii_terminates_at_the_first_whitespace(source in r"[[:ascii:]]+[ \x09-\x0d]") {
            ...
            let r = parse_word_from_ascii(&mut stream);
        ...
        fn parse_unsigned_from_ascii_parses_unsigned_integer_numbers(expected: u64, nl in r"[ \x09-\x0d]") {
            ...
            let r = parse_unsigned_from_ascii::<u64, _>(&mut stream);
