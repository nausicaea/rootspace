use crate::{Resources, Resource, MaybeDefault, ResourceRegistry};
use log::debug;
...
{
    if RR::LEN > 0 {
        maybe_default::<RR::Head>(
            resources,
            PhantomData::default(),
        );

        initialize_recursive::<RR::Tail>(
            resources,
            PhantomData::default(),
        );
    }
