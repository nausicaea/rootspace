...
mod tests {
    ...
    fn parse_ply_fails_with_garbage() {
        ...
        let r = dbg_dmp(parse_ply, "GARBAGE")(&input[..]);
    ...
    fn parse_ply_fails_with_incomplete_header() {
        ...
        let r = nom::error::dbg_dmp(parse_ply, "INCOMPLETE HEADER")(&input[..]);
        assert!(r.is_err(), "{:?}", r.unwrap());
        print!("{:?}", r.unwrap_err())
    }
    ...
    fn parse_ply_succeeds_for_ascii_cube() {
        ...
        let r = dbg_dmp(parse_ply, "ASCII CUBE")(&input[..]);
    ...
    fn parse_ply_succeeds_for_large_ascii_file() {
        ...
        let r = dbg_dmp(parse_ply, "ASCII BUN ZIPPER")(&input[..]);
        assert!(r.is_ok(), "{:?}", r.unwrap_err());
    }
    ...
    fn parse_ply_succeeds_for_large_binary_little_endian_file() {
        ...
        let r = dbg_dmp(parse_ply, "LE FILE")(&input[..]);
        assert!(r.is_ok(), "{:?}", r.unwrap_err());
    }
}
