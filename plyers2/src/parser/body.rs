use nom::{
    ...
    IResult, number::complete::{
        le_u8, le_i8, le_u16, le_i16, le_u32, le_i32, le_f32, le_f64,
        be_u8, be_i8, be_u16, be_i16, be_u32, be_i32, be_f32, be_f64,
    },
};

use super::ParseNumError;
use super::common::{is_whitespace, whitespace};
use crate::types::{DataType, ElementDescriptor, FormatType, CountType, PlyDescriptor};
...
fn list_properties_fct<'a, F1, F2, P1, P2>(cnt_fn: &'a F1, num_fn: &'a F2, types: Vec<(CountType, DataType)>) -> impl FnMut(&'a [u8]) -> IResult<&'a [u8], Vec<u8>> 
where
...
fn elements_fct<'a, F1, F2, P1, P2>(cnt_fn: &'a F1, num_fn: &'a F2, elements: Vec<ElementDescriptor>) -> impl FnMut(&'a [u8]) -> IResult<&'a [u8], (Vec<u8>, Vec<u8>)> 
where
    P1: FnMut(&'a [u8]) -> IResult<&'a [u8], usize>,
    P2: FnMut(&'a [u8]) -> IResult<&'a [u8], Vec<u8>>,
    F1: Fn(CountType) -> P1,
    F2: Fn(DataType) -> P2,
{
    ...
    fold_many_m_n(m, m, move |input| {
        ...
        let r = pair(
            ...
            list_properties_fct(cnt_fn, num_fn, el.list_properties.iter().map(|p| (p.count_type, p.data_type)).collect())
        )(input);
        i += 1;
        r
    },
