    ...
    N: Scalar + RealField,
{
    ...
    pub fn decompose(value: &Affine3<N>) -> Self {
        ...
        let s = Vector3::new(
            value.matrix().column(0).norm(),
            value.matrix().column(1).norm(),
            value.matrix().column(2).norm(),
        );
    ...
    N: Scalar + RealField,
{
    ...
    fn from(value: Affine3<N>) -> Self {
        AffineTransform::decompose(&value)
    }
}
...
mod tests {
    ...
    fn inverse() {
        let a: AffineTransform<f32> = AffineTransform::identity();
        let b: AffineTransform<f32> = AffineTransform::from_parts(
            Translation3::from(Vector3::new(0.1, 0.2, 0.3)),
            UnitQuaternion::identity(),
            Vector3::new(1.0, 2.0, 2.5),
        );
        let c: AffineTransform<f32> = AffineTransform::from_parts(
            Translation3::from(Vector3::new(-0.1, -0.2, -0.3)),
            UnitQuaternion::identity(),
