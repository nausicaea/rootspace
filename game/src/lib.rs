...
impl Game {
    ...
    pub fn run(&mut self, headless: bool, iterations: Option<usize>) -> Result<(), Error> {
        ...
        self.context_mut().add(test_entity, Model::default()).unwrap();
        ...
        if headless {
            ...
            self.context_mut().add(test_entity, HeadlessRenderData::triangle(&renderer.backend).unwrap()).unwrap();
            ...
            self.context_mut().add(test_entity, GliumRenderData::triangle(&renderer.backend).unwrap()).unwrap();
